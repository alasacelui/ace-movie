<template>
  <div class="mt-5">
    <v-container class="px-10">
      ...
      <v-row>
        <v-col>
          <h1 class="subheader light-blue--text">Popular Movies</h1>
        </v-col>
      ...
      <v-row class="py-5" v-if="popular_movies.length">
...
export default {
  ...
  data() {
    return {
      ...
      showMovieInfo: [],
      search_movie: '',
      query_param: 'movie/popular' //search/movie
    }
  },
  methods: {
    async getPopularMovies(page) {
        const res = await fetch(`https://api.themoviedb.org/3/movie/popular?api_key=59b2f04f78d1977273c115fc826eb437&language=en-US&page=${page}`)
        const {results} = await res.json()
        this.popular_movies = results
    },
    handlePagination(value) {
      this.page = value

      if(this.query_param == 'movie/popular') {  // check if its comming from the search box or its comming from the default page
    ...
    if(this.search_movie) 
    {
      ...
      this.popular_movies = results.results
