<template>
  <div class="mt-5">
    <v-container class="px-10">
      ...
      <v-row>
        <v-col>
          <h1 class="subheader light-blue--text">Upcoming Movies</h1>
        </v-col>
      ...
      <v-row class="py-5" v-if="upcoming_movies.length">
            <v-col class="d-flex"  :class="{ custom2cols: $vuetify.breakpoint.xs,
                                            ...
                                            v-for="movie in upcoming_movies" :key="movie.id"
...
export default {
  ...
  data() {
    return {
      ...
      showMovieInfo: [],
      search_movie: '',
      query_param: 'movie/upcoming' //search/movie
    }
  },
  methods: {
    async getUpcomingMovies(page) {
        const res = await fetch(`https://api.themoviedb.org/3/movie/upcoming?api_key=59b2f04f78d1977273c115fc826eb437&language=en-US&page=${page}`)
        const {results} = await res.json()
        this.upcoming_movies = results
    },
   async handleSearch(page) {
      ...
      if(this.search_movie) 
      {
        ...
        this.upcoming_movies = results.results
